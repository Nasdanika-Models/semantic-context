package org.nasdanika.models.semanticcontext.ecore;
...
@EPackageNodeProcessorFactory(nsURI = SemanticContextPackage.eNS_URI)
public class EcoreGenSemanticContextProcessorsFactory {

	private Context context;
	
	public EcoreGenSemanticContextProcessorsFactory(Context context) {
		this.context = context;
	}
		
	@EPackageNodeProcessorFactory(
			label = "Semantic Context Model",
//			description = "",
			actionPrototype = """
                    app-action:
                      content:
		...
		return new EPackageNodeProcessor(config, context, prototypeProvider) {
			...
			protected void generateDiagramAndGraphActions(Collection<Label> labels, ProgressMonitor progressMonitor) {
				...
				try {
					...
					Files.writeString(new File("target/semantic-context.drawio").toPath(), document.save(null));
				} catch (Exception e) {
					e.printStackTrace();
